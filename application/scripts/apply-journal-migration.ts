/**
 * Applies the journal migration (0003_node_journal.sql) using the postgres-js
 * client.
 *
 * Usage: pnpm tsx scripts/apply-journal-migration.ts
 *
 * Reads $DATABASE_URL_DIRECT (or falls back to $DATABASE_URL) and executes the
 * SQL file at drizzle/migrations/0003_node_journal.sql. Idempotent — uses
 * IF NOT EXISTS guards in the SQL.
 *
 * Mirrors scripts/apply-rbac-migration.ts.
 */
...
async function main() {
  ...
  const sqlPath = join(process.cwd(), 'drizzle/migrations/0003_node_journal.sql');
  ...
  try {
    ...
    const tables = await sql<{ tablename: string }[]>`
      ...
      WHERE schemaname = 'public'
        AND tablename = 'node_journal_entries'
    `;
